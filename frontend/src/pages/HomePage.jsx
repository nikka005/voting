          ...
          <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 items-center">
            ...
            <div className="relative hidden lg:block">
              <div className="relative">
                {/* Main Card */}
                {contestants[0] && (
                  <Link to={`/contestant/${contestants[0].slug}`} className="block">
                ...
                {/* Floating Cards */}
                {contestants[1] && (
                  <Link to={`/contestant/${contestants[1].slug}`} className="absolute -right-8 top-20 w-48 rounded-2xl overflow-hidden shadow-xl transform hover:scale-105 transition-transform">
                ...
                {contestants[2] && (
                  <Link to={`/contestant/${contestants[2].slug}`} className="absolute -left-8 bottom-20 w-40 rounded-2xl overflow-hidden shadow-xl transform hover:scale-105 transition-transform">
      ...
      <section className="py-10 sm:py-12 md:py-16 bg-gradient-to-b from-white to-slate-50">
        <div className="max-w-7xl mx-auto px-3 sm:px-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 md:gap-6">
            {trending.map((c, idx) => (
              <Link key={c.id} to={`/contestant/${c.slug}`} className="group">
              ...
              <div className="bg-white rounded-2xl sm:rounded-3xl shadow-xl overflow-hidden">
                ...
                <div className="grid grid-cols-3 gap-2 sm:gap-4 p-4 sm:p-6 bg-gradient-to-r from-amber-50 to-orange-50 border-b border-amber-100">
                  {leaderboard.slice(0, 3).map((entry, idx) => (
                    <Link key={entry.contestant_id} to={`/contestant/${entry.slug}`} className="text-center group">
                ...
                <div className="divide-y divide-slate-100">
                  {leaderboard.slice(3, 10).map((entry, idx) => (
                    <Link key={entry.contestant_id} to={`/contestant/${entry.slug}`} className="flex items-center gap-2 sm:gap-4 p-3 sm:p-4 hover:bg-slate-50 transition-colors">
        ...
        <div className="max-w-7xl mx-auto px-3 sm:px-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4 md:gap-6">
            {contestants.slice(0, 8).map((c, idx) => (
              <Link key={c.id} to={`/contestant/${c.slug}`}>
